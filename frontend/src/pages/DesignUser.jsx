...
const DesignUser = () => {
  ...
  useEffect(() => {
    if (userId) {
      const fetchUserDesignItems = async () => {
        try {
          const response = await axios.get(
            `${import.meta.env.VITE_API_URL}/designusers/${userId}`
          );
          ...
          if (response.data && Array.isArray(response.data.items)) {
            ...
            const itemsWithDetails = response.data.items.map((item) => {
              const { designItemId } = item; // Destructure to access populated catering item
              return {
                ...item, // Contains the user's selected item details
                designItemId: {
                  ...designItemId, // Includes populated details like ItemName, category, and price
                },
              };
            });
          ...
          console.error("Error fetching user's design items:", err);
          setSelectedItems([]);
        }
      };

      fetchUserDesignItems();
    }
  }, [userId]);
  ...
  const handleAddItem = async (item) => {
    ...
    if (existingIndex !== -1) {
      ...
      try {
        ...
        toast.success(
          "Designing service added to your shopping cart successfully!"
        );
      } catch (err) {
        console.error(
          "Error updating item:",
        ...
        toast.error("Failed to update designing item.");
      }
    } else {
      ...
      try {
        await axios.post(`${import.meta.env.VITE_API_URL}/designusers`, {
          userId,
        ...
        toast.error("Failed to add item to designing order.");
      }
    }
  };
  ...
  const handleQuantityChange = async (index, newQuantity) => {
    ...
    try {
      ...
      // Ensure designItemId is a string
      const designItemId =
        updatedItems[index].designItemId?._id || updatedItems[index]._id;

      if (!designItemId || typeof designItemId !== "string") {
        throw new Error("designItemId is missing or not a valid string");
      }
      ...
      const grandTotal = selectedItems.reduce((total, item) => {
        return (
          total +
          (item.designItemId?.price * item.quantity ||
      ...
      toast.success("Designing item quantity updated successfully!");
    } catch (err) {
      console.error(
      ...
      <div>
        ...
        <button
          onClick={async () => {
            ...
            console.log("Deleting item with designItemId:", item.designItemId);

            try {
              // Send DELETE request with userId and designItemId
              await axios.delete(
                `${import.meta.env.VITE_API_URL}/designusers/${userId}/${
                  item.designItemId._id
                }`
              );
              ...
              toast.success(
                "Designing item removed successfully and cart updated!"
              );
            } catch (err) {
              console.error(
                "Error deleting item:",
              ...
              toast.error(
                "Failed to remove Designing item or update shopping cart."
              );
            }
          }}
              ...
              {selectedItems.map((item, index) => (
                ...
                >
                  <td className="border-b lg:p-2 p-1">
                    {item.designItemId?.category || item.category || "N/A"}
                  </td>
                  <td className="border-b lg:p-2 p-1">
                    {item.designItemId?.itemName || item.itemName || "N/A"}
                  </td>
                  <td className="border-b lg:p-2 p-1">
                    <input
                      type="number"
                  ...
                  <td className="border-b lg:p-2 p-1">
                    {item.designItemId?.price || item.price || "N/A"} €
                  </td>
                  <td className="border-b lg:p-2 p-1">
                    {item.designItemId?.price * item.quantity ||
                      item.price * item.quantity ||
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 text-BgFont text-center">
        {Array.isArray(currentItems) && currentItems.length > 0 ? (
          currentItems.map((item) => (
            <div
              key={item.designItemId || item._id}
